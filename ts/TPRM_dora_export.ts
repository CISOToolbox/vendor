...
var _FX_TO_EUR: Record<string, number> = {
...
var _fx: Record<string, number> = _FX_TO_EUR;

function _convert(amount: any, src: any, target: any) {
...
function _eba(table: any, value?: string) {
...
function _ebaFromCodelist(key: any, value?: string) {
    if (value === null || value === undefined || value === "") return "";
...
// ── Sheet writer ──────────────────────────────────────────────────
function _writeSheet(wb: any, name: any, cols: any, rows: any) {
    ...
    hdrRow1.height = 32;
    hdrRow1.eachCell(function(cell: any) {
        cell.font = { bold: true };
        cell.fill = { type: "pattern", pattern: "solid", fgColor: { argb: "FFDDDDDD" } };
        cell.alignment = { horizontal: "left", vertical: "middle", wrapText: true };
    });
    var hdrRow2 = ws.getRow(2);
    hdrRow2.eachCell(function(cell: any) {
...
function _stamp() {
    var d = new Date();
    function pad(n: any) { return ("0" + n).slice(-2); }
    return d.getFullYear() + pad(d.getMonth() + 1) + pad(d.getDate())
        + "_" + pad(d.getHours()) + pad(d.getMinutes()) + pad(d.getSeconds());
}

function _say(msg: any) {
...
// ── Main entry point ──────────────────────────────────────────────
function _doraExportEBA(tree: any, codelists: any, targetCurrency: any) {
    if (!tree) {
...
function _build(tree: any, codelists: any, targetCurrency: any) {
    ...
    if (meta.fx_rates && typeof meta.fx_rates === "object") {
        for (k in meta.fx_rates) {
            if (meta.fx_rates.hasOwnProperty(k)) {
                var v: any = parseFloat(meta.fx_rates[k]);
    ...
    // Indexes
    var vendorById: Record<string, any> = {};
    var vendorByLei: Record<string, any> = {};
    var i;
    ...
    // Used vendor ids
    var usedVendorIds: Record<string, any> = {};
    ...
    var vendorTotalById: Record<string, any> = {};
    var vendorCurById: Record<string, any> = {};
    var curCount: Record<string, any> = {};
    for (i = 0; i < arrangements.length; i++) {
    ...
    var arrRefById: Record<string, any> = {};
    for (i = 0; i < arrangements.length; i++) {
    ...
    var rfeLeiSet: Record<string, any> = {};
    for (i = 0; i < entities.length; i++) {
