...
(function () {
    ...
    function dateValid(s) {
        if (!s || typeof s !== "string" || !DATE_RE.test(s))
            return false;
    ...
    function nonNegative(v) {
        if (v === null || v === undefined || v === "")
            return true; // empty is OK; "required" is a separate check
    ...
    function ebaCodeValid(value, codelistKey) {
        ...
        for (var i = 0; i < arr.length; i++) {
            ...
            if (!entry)
                continue;
            if (entry.eba_code && String(entry.eba_code).toUpperCase() === v)
                return true;
            if (entry.code && String(entry.code).toUpperCase() === v)
                return true;
        }
        return false;
    }
