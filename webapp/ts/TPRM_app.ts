...
function _vendorLabel(v: any): string {
...
function _ncSpec(): CtNcLocalSpec {
    return {
        ...
        setMaxDays: function(days: number) {
            if (!D.metadata) D.metadata = { organization: "", created: _today() };
        ...
        items: {
            options: function() {
                return (D.vendors || []).map(function(v: any) { return { id: v.id, label: _vendorLabel(v) }; });
            },
        ...
        measures: {
            ...
            open: function(id) {
                var vi = (D.vendors || []).findIndex(function(v: any) {
                    return (v.measures || []).some(function(m: any) { return m.id === id; });
                });
                if (vi < 0) return;
                var mi = (D.vendors[vi].measures || []).findIndex(function(m: any) { return m.id === id; });
...
function _createVendorForNc(draft: { title: string; description: string; domain: string }): Promise<CtNcItemOption | null> {
    ...
    return window.ct_modal.open({ title: t("tprm.nc.create_vendor"), body: h, size: "md", buttons: [
        ...
        while ((D.vendors || []).some(function(x: any) { return x.id === v.id; })) {
