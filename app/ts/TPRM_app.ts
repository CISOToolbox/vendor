...
function _offboardVendor(v: any): void {
    ...
    var gardees = mesures.filter(function(m: any) { return TRACE_MESURE.indexOf(m.statut) >= 0; });
    var perdues = mesures.length - gardees.length;
    v.measures = gardees;

    var risquesAvant = (D.risks || []).filter(function(r: any) { return r.vendor_id === v.id; }).length;
    D.risks = (D.risks || []).filter(function(r: any) {
        return r.vendor_id !== v.id || TRACE_RISQUE.indexOf(r.status) >= 0;
    });
    var risquesPerdus = risquesAvant - (D.risks || []).filter(function(r: any) { return r.vendor_id === v.id; }).length;
...
function renderVendorDetail() {
    ...
    if (_hdrRisks.length > 0) {
        if (menace > 0) h += '<span style="width:1px;height:14px;background:var(--ct-line)"></span>';
