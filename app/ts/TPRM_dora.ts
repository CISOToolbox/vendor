...
var _doraTree: DoraTree | null = null;
var _doraCodelists: DoraCodelists | null = null;
var _doraExportCurrency = "EUR";

function _doraT(key: string, fallback?: string) {
    var v = (typeof t === "function") ? t(key) : null;
    if (v && v !== key) return v;
    return fallback || key;
}

function _newId(prefix: string) {
...
function _doraNextRef(prefix: string) {
...
function _codelistOptions(key: string, current: string | null | undefined) {
...
function _countryOptions(current: string | null | undefined) {
...
function _renderEntityOverview() {
    ...
    var fnUsage: Record<string, any> = {};
    arrs.forEach(function(a) {
        (a.function_ids || []).forEach(function(fid: any) {
        ...
        rfes.forEach(function(r) {
            ...
            var parentField = needsParent
                ? _fld(_doraT("dora.overview.f.parent_lei", "LEI de la maison-mère (B_01.02.0060)"),
                    '<div class="ct-flex ct-gap-1 ct-items-center"><input id="rfe-plei-' + esc(r.id) + '" value="' + esc(r.parent_lei || "") + '" maxlength="20" data-input="doraPatchEntity" data-args=\'["' + esc(r.id) + '","parent_lei"]\' data-pass-value class="ct-flex-1 ct-mono ct-upper">' + _gleifTriggerHtml("rfe-plei-" + r.id) + '</div>')
                : '';
    ...
    function _rfeOptsForBranch(currentRfeId: string) {
...
function _renderVendorCard(v: TprmVendor, opts: any) {
    if (!v) return "";
    opts = opts || {};
    var embedded = !!opts.embedded;
    var fns = _doraTree!.functions || [];
    var fnById: Record<string, any> = {};
    ...
    var fnIds: Record<string, any> = {};
    vArrs.forEach(function(a) { (a.function_ids || []).forEach(function(id: any) { if (id) fnIds[id] = true; }); });
    ...
    if (!embedded) {
        ...
        if (v.lei) h += '<code class="ct-text-label ct-muted">' + esc(v.lei) + '</code>';
        if (v.country_iso2) h += '<span class="ct-text-meta ct-muted">' + esc(v.country_iso2) + '</span>';
        if (roi.complete) {
    ...
    if (vArrs.length === 0) {
        ...
        vArrs.forEach(function(a) {
            ...
            var aFns = (a.function_ids || []).map(function(id: any) {
                var f = fnById[id]; return f ? (f.name || f.id) + (f.is_critical_or_important ? " ★" : "") : id;
            });
            ...
            if (a.arrangement_type) h += '<span class="ct-text-label ct-muted ct-bg-alt ct-p-1 ct-r-sm">' + esc(_doraCodeLabel("arrangement_type", a.arrangement_type)) + '</span>';
            if (a.is_critical_function_support) h += '<span class="ct-text-label ct-kpi-tone ct-text-onsolid ct-py-1 ct-px-1 ct-r-sm">★ ' + esc(_doraT("dora.byvendor.critical", "critical")) + '</span>';
            h += '<span class="ct-flex-1"></span>';
            ...
            if (aFns.length || aSubs.length) {
                ...
                if (aFns.length === 0) h += '<span class="ct-muted">—</span>';
                else h += aFns.map(function(name: any) { return '<span style="display:inline-block;padding:var(--ct-s1) var(--ct-s1);margin:var(--ct-s1);background:var(--ct-surface-2);border-radius:var(--ct-r-sm);font-size:var(--ct-text-ui)">' + esc(name) + '</span>'; }).join("");
                h += '</div>';
                ...
                if (aSubs.length === 0) h += '<span class="ct-muted">—</span>';
                else {
    ...
    if (fnList.length) h += fnList.map(function(name) { return '<span style="display:inline-block;padding:var(--ct-s1) var(--ct-s1);margin:var(--ct-s1);background:var(--ct-surface-2);border-radius:var(--ct-r-sm)">' + esc(name) + '</span>'; }).join("");
    else h += '<span class="ct-muted">—</span>';
    h += '</div>';
...
window.doraPatchEntity = function(id: string, field: string, value: string) {
...
window.doraPatchEntityNum = function(id: string, field: string, value: string) {
    var v = value === "" ? null : parseFloat(value);
    var r = _doraTree!.entities.find(function(x) { return x.id === id; });
    if (r) r[field] = v;
    var f: Record<string, any> = {}; f[field] = v;
    _persist("dora_entity", id, f);
};

window.doraDelEntity = function(id: string) {
...
window.doraPatchFn = function(id: string, field: string, value: string) {
...
window.doraPatchFnBool = function(id: string, field: string, evt: Event, el: HTMLInputElement) {
    var v = el.checked;
    var r = _doraTree!.functions.find(function(x) { return x.id === id; });
    if (r) r[field] = v;
    var f: Record<string, any> = {}; f[field] = v;
...
window.doraPatchFnNum = function(id: string, field: string, value: string) {
    var v = value === "" ? null : parseFloat(value);
    var r = _doraTree!.functions.find(function(x) { return x.id === id; });
    if (r) r[field] = v;
    var f: Record<string, any> = {}; f[field] = v;
...
window.doraDelFn = function(id: string) {
...
window.doraPatchBranch = function(id: string, field: string, value: string) {
...
window.doraDelBranch = function(id: string) {
...
window.doraPatchCs = function(id: string, field: string, value: string) {
...
window.doraDelCs = function(id: string) {
...
window.doraPatchArr = function(id: string, field: string, value: string) {
...
window.doraPatchArrBool = function(id: string, field: string, evt: Event, el: HTMLInputElement) {
    var v = el.checked;
    var r = _doraTree!.arrangements.find(function(x) { return x.id === id; });
    if (r) r[field] = v;
    var f: Record<string, any> = {}; f[field] = v;
...
window.doraPatchArrNum = function(id: string, field: string, value: string) {
    var v = value === "" ? null : parseFloat(value);
    var r = _doraTree!.arrangements.find(function(x) { return x.id === id; });
    if (r) r[field] = v;
    var f: Record<string, any> = {}; f[field] = v;
...
window.doraDelArr = function(id: string) {
...
window.doraOpenVendorRoi = function(vendorId: string) {
    var vendors = (window.D && D.vendors) || [];
    var idx = -1;
...
window.doraAddSigner = function() {
    ...
    if (!sel || !(sel as HTMLInputElement).value) return;
    var aid = (sel as HTMLInputElement).value;
    var id = _newId("SIG");
...
window.doraPatchSigner = function(compositeId: string, field: string, value: string) {
...
window.doraDelSigner = function(compositeId: string) {
    var p = compositeId.split("/");
    var aid = p[0], sid = p[1];
...
function _doraVendorSelfSignerRow(vendor: TprmVendor | null, arrangementId: string, sortOrder: number) {
    return {
        id: _newId("SIG"),
...
function _doraIsVendorSelfSigner(s: DoraRecord | null | undefined, vendor: TprmVendor | null | undefined) {
...
function _doraDistinctSignersForVendor(vendorId: string, excludeArrId: string | null) {
    var arrs = (_doraTree!.arrangements || []).filter(function(a) {
        return a.vendor_id === vendorId && a.id !== excludeArrId;
    });
    var arrIds: Record<string, any> = {};
    arrs.forEach(function(a) { arrIds[a.id] = true; });
    var seen: Record<string, any> = {};
    var out: { signer_lei: string; signer_name: string; signer_role: string }[] = [];
...
window.doraAddSignerByExisting = function(arrangementId: string) {
    var sel = document.getElementById("arr-signer-pick");
    if (!sel || !(sel as HTMLInputElement).value) return;
    var vendor: any = (_doraTree!.arrangements || []).find(function(a) { return a.id === arrangementId; });
    ...
    var src = _doraDistinctSignersForVendor(vendor ? vendor.id : "", null)
        .filter(function(x) {
            ...
            return key === (sel as HTMLInputElement).value;
        })[0];
    if (!src) return;
...
window.doraRemoveSignerFromArr = function(compositeId: string) {
    var p = compositeId.split("/");
    var aid = p[0], sid = p[1];
    _doraTree!.signers = (_doraTree!.signers || []).filter(function(x) { return !(x.arrangement_id === aid && x.id === sid); });
    _persistDelete("dora_signer", compositeId);
...
window.doraPatchArrSigner = function(compositeId: string, field: string, value: string) {
    var p = compositeId.split("/");
    var aid = p[0], sid = p[1];
    var r = (_doraTree!.signers || []).find(function(x) { return x.arrangement_id === aid && x.id === sid; });
    if (r) r[field] = value;
...
window.doraOnSignerModeChange = function(arrangementId: string) {
    _doraRefreshArrSignerSection(arrangementId);
};

// Re-render the in-modal signer block in place (no global render).
function _doraRefreshArrSignerSection(arrangementId: string) {
...
function _doraRenderArrSignerSlot(arrangementId: string, modeOverride?: string | null) {
    var mode;
    if (modeOverride === "vendor_self" || modeOverride === "other") {
        mode = modeOverride;
    } else {
        var modeEl = document.querySelector('input[name="arr-signer-mode"]:checked');
        mode = modeEl ? (modeEl as HTMLInputElement).value : "other";
    }
...
// modal stack pattern).
window.doraOpenSignerModalForArr = function(arrangementId: string) {
...
window.doraOpenSignerCreateModal = function(arrangementId: string) {
    ...
    var buttons: CtModalButton[] = [
        ...
        { id: "save", label: _doraT("btn_save", "Save"), primary: true, result: function() {
            var name = ((document.getElementById("sig-name") as HTMLInputElement | null) || ({} as HTMLInputElement)).value || "";
            name = name.trim();
            if (!name) { showStatus(_doraT("dora.signer.name_required", "Signer name is required.")); return false; }
            var lei = (((document.getElementById("sig-lei") as HTMLInputElement | null) || ({} as HTMLInputElement)).value || "").trim().toUpperCase();
            var role = ((document.getElementById("sig-role") as HTMLInputElement | null) || ({} as HTMLInputElement)).value || "";
            var existing = (_doraTree!.signers || []).filter(function(s) { return s.arrangement_id === arrangementId; });
            var row = {
                id: _newId("SIG"),
                arrangement_id: arrangementId,
...
function _renderSubcontractors() {
    ...
    var vById: Record<string, any> = {}; (_doraTree!._vendors || []).forEach(function(v: any) { vById[v.id] = v; });
...
window.doraNewSubFromLink = function(arrangementId: string) {
    ...
    setTimeout(function() {
        ...
        var poll = setInterval(function() {
            var ov = document.querySelector<HTMLElement>(".ct-modal-overlay");
...
window.doraPatchSub = function(subId: string, field: string, value: string) {
...
window.doraDelSub = function(subId: string) {
    ...
    (_doraTree!.arrangements || []).forEach(function(a) {
        if (a.subcontractor_links) {
            a.subcontractor_links = a.subcontractor_links.filter(function(l: any) { return l.subcontractor_id !== subId; });
        }
    });
...
window.doraUnlinkSub = function(cid: string) {
    ...
    if (a && a.subcontractor_links) {
        a.subcontractor_links = a.subcontractor_links.filter(function(x: any) { return x.subcontractor_id !== sid; });
    }
    _persistDelete("dora_sub_link", cid);
    _render(document.getElementById("dora-root")!);
};

window.doraLinkSub = function(arrangementId: string, subId: string | null, perLinkFields: any) {
...
window._doraValid = {
    lei: function(v: unknown) { return typeof v === "string" && /^[A-Za-z0-9]{18}[0-9]{2}$/.test(v) && window._doraValid!.leiChecksum(String(v)); },
    leiChecksum: function(lei: string) {
        var u = (lei || "").toUpperCase();
        if (!/^[A-Z0-9]{18}[0-9]{2}$/.test(u)) return false;
        var expanded = "";
    ...
    country: function(v: unknown) {
        if (typeof v !== "string" || !/^[A-Za-z]{2}$/.test(v)) return false;
        var items = (_doraCodelists && _doraCodelists.country_iso3166_1) || [];
        if (!items.length) return true;   // codelists pas encore chargées — soft
        return items.some(function(it: any) { return String(it.code !== undefined ? it.code : it).toUpperCase() === v.toUpperCase(); });
    },
    currency: function(v: unknown) {
        if (typeof v !== "string" || !/^[A-Za-z]{3}$/.test(v)) return false;
        var items = (_doraCodelists && _doraCodelists.currency_iso4217) || [];
        if (!items.length) return true;
        return items.some(function(it: any) { return String(it.code !== undefined ? it.code : it).toUpperCase() === v.toUpperCase(); });
    },
    date: function(v: unknown) { return typeof v === "string" && /^\d{4}-\d{2}-\d{2}$/.test(v); },
    reportingPeriod: function(v: unknown) { return typeof v === "string" && /^\d{4}-12-31$/.test(v); },
    nonNegative: function(v: unknown) { var n = Number(v); return isFinite(n) && n >= 0; },
    ebaCode: function(value: unknown, codelistKey: string) {
        var items = (_doraCodelists && _doraCodelists[codelistKey]) || [];
        return items.some(function(it: any) { return String(it.code !== undefined ? it.code : it) === String(value); });
    }
};
...
window._validateDoraROI = function(): Promise<{ ok: boolean; errors: Array<Record<string, string>> }> {
    var pid = window.getActiveProjectId && window.getActiveProjectId();
    if (!pid) return Promise.resolve({ ok: false, errors: [{ kind: "project", id: "", label: "", message: _doraT("dora.no_project", "Open or create a project first.") }] });
...
window._showDoraValidationModal = function(errors: Array<Record<string, string>>, onProceed?: () => void) {
    ...
    if (onProceed) buttons.push({ id: "force", label: _doraT("dora.valid.force", "Exporter quand même"), danger: true, result: true });
    ...
    }).then(function(r) { if (r === true && onProceed) onProceed(); });
};
        ...
        window.ct_modal!.open({
            ...
            buttons: [
                ...
                { id: "ok", label: _doraT("dora.export.download", "Télécharger XLSX"), primary: true, result: function() {
                    ...
                    if (sel) _doraExportCurrency = (sel as HTMLInputElement).value;
                    var rpInput = document.getElementById("dora-export-rp");
                    var rpVal = rpInput ? ((rpInput as HTMLInputElement).value || "").trim() : "";
...
// is localised.
function _doraCodeI18n(key: string, code: string, jsonLabel?: string) {
...
function _doraCodeItems(key: string) {
    var items = (_doraCodelists && _doraCodelists[key]) || [];
...
function _doraCodeLabel(key: string, code?: string) {
    if (!code) return "";
    var items = (_doraCodelists && _doraCodelists[key]) || [];
    var jsonLabel = code;
...
function _doraCurrencyItems() {
    ...
    return currencies.map(function(c: any) {
...
function _doraRefValue(uid: string) {
    var dd = document.getElementById(uid + "-dd");
    if (!dd) return "";
    var c = dd.querySelector("input:checked");
    return c ? (c as HTMLInputElement).value : "";
}

function _doraRefValues(uid: string) {
    var dd = document.getElementById(uid + "-dd");
    if (!dd) return [];
    var out: string[] = [];
    dd.querySelectorAll("input:checked").forEach(function(c) { out.push((c as HTMLInputElement).value); });
    return out;
}

// Manually refresh the tags strip after a programmatic uncheck (no shared
// internal access — we rebuild the strip ourselves).
function _doraRefRefreshTags(uid: string, items: any[], emptyText?: string) {
...
function _doraRefSelect(uid: string, current: string | null | undefined, items: any[], opts?: any) {
    opts = opts || {};
    ...
    window.ctRefRegister(uid, {
        ...
        onRemove: function(u, optId) {
            ...
            if (dd) {
                var inps = dd.querySelectorAll('input[value]') as NodeListOf<HTMLInputElement>;
...
// arrangement modal.
window.doraOpenFunctionModal = function(functionId: string | null, opts: any) {
    ...
    function _b(id: string) { var el = document.getElementById(id) as HTMLInputElement | null; return el ? !!el.checked : false; }
    function _n(v: any) { return v === "" || v == null ? null : Number(v); }

    var buttons: CtModalButton[] = [{ id: "cancel", label: _doraT("btn_cancel", "Cancel") }];
    if (!isNew) {
...
window.doraOpenArrangementModal = function(arrangementId: string, vendorIdHint: string | null, lockVendor: boolean, prefill: any) {
    if (typeof window.ct_modal === "undefined") return;
    ...
    var _otherArrs = ((window._doraTree && _doraTree!.arrangements) || []).filter(function(x: any) { return x.id !== a!.id; });
    ...
    if (!isNew) {
        ...
        var subByIdLocal: Record<string, any> = {}; (_doraTree!.subcontractors || []).forEach(function(s) { subByIdLocal[s.id] = s; });
        var linksTbl = '';
    ...
    function _collect() {
        function _v(id: string) { var el = document.getElementById(id) as HTMLInputElement | null; return el ? el.value : ""; }
        function _b(id: string) { var el = document.getElementById(id) as HTMLInputElement | null; return el ? !!el.checked : false; }
        function _nFromV(v: any) { return v === "" || v == null ? null : Number(v); }
    ...
    buttons.push({ id: "save", label: _doraT("btn_save", "Save"), primary: true, result: function() {
        ...
        var signerMode = signerModeEl ? (signerModeEl as HTMLInputElement).value : "other";
...
window.doraOpenSubIdentityModal = function(subId: string | null) {
    if (typeof window.ct_modal === "undefined") return;
    ...
    var arrById: Record<string, any> = {}; (_doraTree!.arrangements || []).forEach(function(a) { arrById[a.id] = a; });
    var vById: Record<string, any> = {}; (_doraTree!._vendors || []).forEach(function(v: any) { vById[v.id] = v; });
    var linksHtml = '';
    if (links.length === 0) {
...
window.doraOpenSubcontractorModal = function(arrangementId: string, subId: string | null, preselect: any) {
    if (typeof window.ct_modal === "undefined") return;
    if (!arrangementId) return;
    var isNew = !subId;
    ...
    var subById: Record<string, any> = {}; (_doraTree!.subcontractors || []).forEach(function(s) { subById[s.id] = s; });
    ...
    function _collect() {
        function _b(id: string) { var el = document.getElementById(id) as HTMLInputElement | null; return el ? !!el.checked : false; }
        var rankEl = document.getElementById("link-rank") as HTMLSelectElement | null;
        var tier = Math.max(parseInt((rankEl && rankEl.value) as string, 10) || 2, 2);
        return {
            tier: tier,
            parent_subcontractor_id: null,
...
var _gleifPopoverEl: HTMLElement | null = null;
...
function _gleifOutsideClick(ev: MouseEvent) {
    if (!_gleifPopoverEl) return;
    if (_gleifPopoverEl.contains(ev.target as Node | null)) return;
    if ((ev.target as Element | null)?.closest && (ev.target as Element).closest("[data-gleif-trigger]")) return;
    _gleifClosePopover();
}
...
function _gleifOpenPopover(triggerEl: Element, targetInput: HTMLInputElement, onPick?: ((rec: any) => void) | null) {
    _gleifClosePopover();
    ...
    if (targetInput && targetInput.value) (input as HTMLInputElement).value = targetInput.value;
    try { (input as HTMLInputElement).focus(); (input as HTMLInputElement).select(); } catch (e) {}
    ...
    input!.addEventListener("keydown", function(ev) {
        if ((ev as KeyboardEvent).key === "Escape") _gleifClosePopover();
    });

    results!.addEventListener("click", function(ev) {
        var row = (ev.target as Element | null)?.closest(".gleif-row");
        if (!row) return;
        var lei = row.getAttribute("data-lei") || "";
        var name = row.getAttribute("data-name") || "";
        var country = row.getAttribute("data-country") || "";
        var arr = (results as any).__gleifRecords || [];
        var record = arr.find(function(r: any) { return (r.attributes && r.attributes.lei) === lei; }) || null;
        if (targetInput) {
    ...
    if ((input as HTMLInputElement).value && (input as HTMLInputElement).value.trim().length >= 2) search();
...
window.gleifOpenLookup = function(targetInputId: string) {
    ...
    if (!trigger) return;
    _gleifOpenPopover(trigger, input as HTMLInputElement, null);
};
...
window.DoraData!.ensureCodelists = function(cb) {
    ...
    if (!window.VendorAPI || typeof VendorAPI.doraCodelists !== "function") { if (cb) cb(null); return; }
...
window.renderDoraPanel = function(host: HTMLElement) {
    _loadTree(host);
};
...
window.doraToggleHint = function(key: string) {
