...
(function () {
    ...
    function _pid() { return _activeId || ""; }
    window.VendorAPI = {
        ...
        listDerogations: function (filters) {
            var parts = [];
            var f = filters || {};
            Object.keys(f).forEach(function (k) { if (f[k])
    ...
    function _doFlush() {
        ...
        Object.keys(batch).forEach(function (key) {
            ...
            var fail = function (e) { console.error("PATCH " + type + " " + id + " failed:", e); };
    ...
    window._persistCreate = function (entityType, data) {
        ...
        var CREATE_FNS = {
            ...
            measure: function (d) { return VendorAPI.createMeasure(_activeId, d.vendor_id || "", d); },
            ...
            dora_signer: function (d) { return VendorAPI.createDoraSigner(_activeId, d.arrangement_id, d); },
            dora_subcontractor: function (d) { return VendorAPI.createDoraSub(_activeId, d); },
            dora_sub_link: function (d) { return VendorAPI.linkDoraSub(_activeId, d.arrangement_id, d); },
        };
        var fn = CREATE_FNS[entityType];
        if (!fn)
            return Promise.resolve();
        var p = fn(data).catch(function (e) {
        ...
        var clear = function () { if (_pendingCreates[key] === p)
            delete _pendingCreates[key]; };
        p.then(clear, clear);
        return p;
    };
    ...
    window._persistDelete = function (entityType, entityId) {
        ...
        if (fn)
            fn(entityId).catch(function (e) { console.error("DELETE " + entityType + " " + entityId + " failed:", e); });
    };
    ...
    window._appInitCallback = function () {
        ...
        function _retry(fn, attempts, delay) {
        ...
        function _bootFailed() {
            ...
            if (typeof showStatus === "function")
                showStatus(msg);
            if (document.getElementById("vendor-boot-error"))
                return;
        ...
        function _loadAndRender(id) {
            _retry(function () { return VendorAPI.get(id); }, 4, 500).then(function (project) {
                ...
                if (typeof _setDataReady === "function")
                    _setDataReady();
    ...
    document.addEventListener("visibilitychange", function () {
        if (document.visibilityState !== "visible" || !_activeId || _saveTimer)
            return;
        if (typeof _dirty !== "undefined" && Object.keys(_dirty).length)
            return;
        VendorAPI.get(_activeId).then(function (project) {
