...
(function () {
    ...
    style.textContent = [
        ...
        ".ai-panel { display:none; position:fixed; top:0; right:-720px; width:700px; max-width:90vw; height:100vh; background:var(--ct-surface); box-shadow:-4px 0 24px rgba(0,0,0,0.2); z-index:501; transition:right 0.3s; overflow-y:auto; }",
        ".ai-panel.open { display:block; right:0; }",
        ".ai-panel-header { display:flex; align-items:center; justify-content:space-between; padding:14px 16px; background:var(--ct-surface-2); color:var(--ct-ink); border-bottom:1px solid var(--ct-line); position:sticky; top:0; z-index:1; }",
