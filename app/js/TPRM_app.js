...
function renderDashboard() {
    ...
    if (!_derogLoaded)
        _loadDerogations(function () { if (_panel === "dashboard")
            renderPanel(); });
    ...
    if (!topR.length)
        h += '<div class="ct-muted ct-text-meta">' + t("risk.empty") + '</div>';
...
function _renderRiskTimeline() {
    ...
    if (!risks.length)
        return '<div class="ct-muted ct-text-meta">-</div>';
...
function renderSubcontractorList() {
    ...
    subs.forEach(function (s) {
        ...
        if (s.lei)
            h += '<code class="ct-text-label ct-muted">' + esc(s.lei) + '</code>';
        if (s.country_iso2)
            h += '<span class="ct-text-label ct-muted">' + esc(s.country_iso2) + '</span>';
        if (s.sector)
            h += '<span class="ct-badge ct-bg-alt ct-muted" data-tone="neutral">' + esc(s.sector) + '</span>';
        if (critN > 0)
            h += '<span class="ct-ref ct-bg-high-tint ct-text-high-ink" data-size="sm">★ ' + critN + ' CIF</span>';
        h += '</div>';
...
function renderVendorList() {
    ...
    if (window.DoraData && !window.DoraData.getTree()) {
        window.DoraData.ensureLoaded(function (tree) {
            if (tree)
                renderPanel(); // re-render once the tree is ready
        });
    }
    ...
    D.vendors.forEach(function (v, i) {
        ...
        var metrics = [];
        if (riskCount > 0)
            metrics.push('<span class="ct-text-critical ct-strong">' + riskCount + ' ' + t("nav.risks").toLowerCase() + '</span>');
...
function renderVendorDetail() {
    ...
    if (!_derogLoaded)
        _loadDerogations(function () { if (_panel === "vendors" && _selectedVendor !== null)
            renderPanel(); });
    var tier = _getTier(v);
...
function _nextVendorMeasureId(v) {
    ...
    var mk = function (i) { return v.id + ":MES-" + String(i).padStart(2, "0"); };
    while ((v.measures || []).some(function (m) { return m.id === mk(n); }))
        n++;
    return mk(n);
}
function addMeasureForRisk(vendorIdx, riskIdx) {
    ...
    if (!v || !r || !window.ct_measure_modal)
        return;
...
function _aiIncludeMeasures() {
    var el = document.getElementById("ai-include-measures");
    return el ? el.checked : true;
}
function _aiIncludeMeasuresHTML() {
...
function _aiSuggestRisksCustom(vendorIdx, prompt) {
    var v = D.vendors[vendorIdx];
    if (!v || typeof _aiIsEnabled !== "function" || !_aiIsEnabled())
        return;
    ...
    window._aiRenderCards({
        ...
        renderCard: function (s, i) {
            ...
            if (isRiskMode) {
                ...
                h += '<div class="ct-text-label ct-muted ct-mt-1">';
                if (s.type)
                    h += '<span style="background:var(--ct-info-tint);padding:var(--ct-s1) var(--ct-s1);border-radius:var(--ct-r-sm);margin-right:var(--ct-s1)">' + esc(s.type) + '</span>';
                if (s.responsable)
                    h += esc(s.responsable);
                h += '</div>';
            }
            ...
            if (ctx.type === "risks_and_measures") {
                ...
                (s.measures || []).forEach(function (m) {
                    ...
                    if ((m.action === "enrich" || m.action === "link") && m.id) {
                        ...
                        if (dejaV) {
                            if (m.action === "enrich") {
                                if (m.details)
                                    dejaV.details = _mergeMeasureDetails(dejaV.details || "", m.details);
                                var ntv = (m.mesure || m.measure || "").trim();
                                if (ntv && ntv !== dejaV.mesure)
                                    dejaV.mesure = ntv;
                ...
                if (s.action === "enrich") {
                    ...
                    if (nt && nt !== cible.mesure)
                        cible.mesure = nt;
                    if (!cible.responsable && s.responsable)
                        cible.responsable = s.responsable;
...
function _renderVendorAssessments(v) {
    ...
    if (!assessments.length)
        return h + '<div class="ct-muted ct-text-meta">' + t("assessment.empty") + '</div>';
...
function _renderDocsTable(docs, tableId) {
    ...
    docs.forEach(function (d) {
        ...
        if (d.url)
            h += '<a href="' + esc(d.url) + '" target="_blank" rel="noopener" class="ct-text-accent" data-stop>&#8599;</a>';
        h += '</div></td>';
        // Expiry date
...
function renderRiskList() {
    ...
    if (!filtered.length)
        return h + '<div class="ct-muted ct-text-meta ct-p-2">' + t("vendor.no_results") + '</div>';
...
function openAssessmentV2(assessId) {
    ...
    if (a.status === "pending_approval") {
        var _apN = (a.responses || []).reduce(function (n, r) { return n + ((r.action_plans || []).length); }, 0);
        if (_apN > 0) {
...
function aiSuggestSectionV2(assessId, sectionId) {
    ...
    if (!a || !a.template_snapshot || typeof _aiIsEnabled !== "function" || !_aiIsEnabled())
        return;
    ...
    var ANS2COV = { compliant: "covered", partial: "partial", non_compliant: "not_covered", na: "not_applicable" };
...
function _copyEmailTemplate(assessId) {
    ...
    var htmlClipboard = '<p style="margin:0 0 var(--ct-s2);font-weight:bold">' + esc(subject) + '</p>' +
        htmlBody;
...
function renderGlobalMeasures() {
    ...
    D.vendors.forEach(function (v) {
        ...
        (v.measures || []).forEach(function (m) {
            if (!allLinkedIds[m.id] && m.statut !== "termine" && m.statut !== "annule")
                unlinkedCount++;
        });
    });
...
function editMeasure(vendorIdx, measureIdx, _returnTo) {
    // Shared modal everywhere — the legacy full-page form is gone.
    if (window._editVendorMeasureRow)
        window._editVendorMeasureRow({ vendorIdx: vendorIdx, measureIdx: measureIdx });
}
...
function _vendorCountryField(value) {
...
function _vendorSectorField(value) {
...
window.AI_APP_CONFIG = {
    ...
    onSettingsRendered: function () {
        _wireDoraSettings();
        _wireCustomQuestionnaire();
        if (typeof _wireDemoSettings === "function")
            _wireDemoSettings();
    },
...
function _scopedKey(vendorId, measureId) {
...
function _ncOptions() {
    return {
        ...
        items: {
            options: function () {
                if (!(window.getActiveProjectId && window.getActiveProjectId()))
                    return [];
                return (D.vendors || []).map(function (v) {
        ...
        measures: {
            ...
            open: function (id) {
                ...
                var vi = (D.vendors || []).findIndex(function (v) { return v.id === vendorId; });
                if (vi < 0)
                    return;
                var mi = ((D.vendors[vi].measures) || []).findIndex(function (m) { return m.id === measureId; });
                if (mi < 0)
                    return;
...
function renderAll() {
    ...
    if (tr) {
        ...
        if (!existing) {
            ...
            if (settingsHtml)
                tr.insertAdjacentHTML("afterbegin", '<span class="toolbar-settings">' + settingsHtml + '</span>');
        }
    }
