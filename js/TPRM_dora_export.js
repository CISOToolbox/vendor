...
(function () {
    ...
    function _convert(amount, src, target) {
        ...
        if (srcEur === undefined || srcEur === null || tgtEur === undefined || tgtEur === null || tgtEur === 0)
            return null;
    ...
    function _ebaFromCodelist(key, value) {
        ...
        for (var i = 0; i < items.length; i++) {
            if (String(items[i].code) === s)
                return items[i].eba_code || "";
        }
        return "";
    }
    ...
    function _build(tree, codelists, targetCurrency) {
        ...
        var k;
        for (k in _FX_TO_EUR)
            if (_FX_TO_EUR.hasOwnProperty(k))
                _fx[k] = _FX_TO_EUR[k];
        ...
        for (i = 0; i < vendors.length; i++) {
            ...
            if (vendors[i].lei)
                vendorByLei[String(vendors[i].lei).toUpperCase()] = vendors[i];
        }
        var fnById = {};
        for (i = 0; i < functions_.length; i++)
            fnById[functions_[i].id] = functions_[i];
        var rfeById = {};
        for (i = 0; i < entities.length; i++)
            rfeById[entities[i].id] = entities[i];
        var subById = {};
        for (i = 0; i < subcontractors.length; i++)
            subById[subcontractors[i].id] = subcontractors[i];
        var arrById = {};
        for (i = 0; i < arrangements.length; i++)
            arrById[arrangements[i].id] = arrangements[i];
        // Used vendor ids
        var usedVendorIds = {};
        for (i = 0; i < arrangements.length; i++) {
            if (arrangements[i].vendor_id)
                usedVendorIds[arrangements[i].vendor_id] = true;
        }
        ...
        for (i = 0; i < arrangements.length; i++) {
            ...
            if (a.annual_cost_amount === null || a.annual_cost_amount === undefined || a.annual_cost_amount === "")
                continue;
            ...
            if (!curCount[a.vendor_id])
                curCount[a.vendor_id] = {};
            curCount[a.vendor_id][cur] = (curCount[a.vendor_id][cur] || 0) + 1;
        }
        for (var vid in curCount) {
            if (!curCount.hasOwnProperty(vid))
                continue;
            var bucket = curCount[vid];
            var best = "", bestN = -1;
            for (var c in bucket) {
                if (bucket.hasOwnProperty(c) && bucket[c] > bestN) {
                    best = c;
                    bestN = bucket[c];
                }
            }
        ...
        var holder = null;
        for (i = 0; i < entities.length; i++) {
            if ((entities[i].hierarchy || "") === "parent") {
                holder = entities[i];
                break;
            }
        }
        if (!holder && entities.length > 0)
            holder = entities[0];
        var holderRows = [];
        if (holder) {
            holderRows.push([
        ...
        for (i = 0; i < entities.length; i++) {
            if (entities[i].lei)
                rfeLeiSet[String(entities[i].lei).toUpperCase()] = true;
        }
        var b0203 = [];
        for (i = 0; i < arrangements.length; i++) {
            ...
            if (v2.ultimate_parent_id) {
                ...
                if (pv && pv.lei && rfeLeiSet[String(pv.lei).toUpperCase()])
                    isIntra = true;
            }
            if (!isIntra)
                continue;
        ...
        for (i = 0; i < signers.length; i++) {
            var s = signers[i];
            if (String(s.signer_role || "").toLowerCase() === "tpp")
                continue;
        ...
        for (i = 0; i < signers.length; i++) {
            var st = signers[i];
            if (String(st.signer_role || "").toLowerCase() !== "tpp")
                continue;
